...
//不直接用PageName来添加是为了分开 app和class
/**
  * @brief  Install the page, and register the page to the page pool
  * @param  className: 页面的类，用于具体的实现
  * @param  appName: 页面应用程序名称，不允许重复
  * @retval Return true if successful
  */
bool PageManager::Install(const char* PageClassName, const char* PageName)
{
    //工厂为空
    if (_Factory == nullptr)
    {
    ...
    //未设置 设置PageName为PageClassName
    if (PageName == nullptr)
    {
        PM_LOG_WARN("PageName has not set");
        PageName = PageClassName;
    }
    //检查页面是否注册
    if (FindPageInPool(PageName) != nullptr)
    {
        PM_LOG_ERROR("Page(%s) was registered", PageName);
        return false;
    }
    //继承PageBase，此函数返回的是继承后的类型
    PageBase* base = _Factory->CreatePage(PageClassName);
    if (base == nullptr)
    {
        //PM_LOG_ERROR("Factory has not %s", className);
        PM_LOG_ERROR("There has no Class name is %s", PageClassName);
        return false;
    }
    base->_root = nullptr;
    base->_ID = 0;
    ...
    PM_LOG_INFO("Install Page[class = %s, name = %s]", PageClassName, PageName);
    bool retval = Register(base, PageName);//注册

    base->onCustomAttrConfig();//执行虚函数
